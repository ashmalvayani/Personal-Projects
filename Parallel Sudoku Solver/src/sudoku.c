...
typedef gridStruct* grid_t;

#define CurrValue(grid) (grid->matrix[grid->i])
...
int checkValidGrid(grid_t grid);
...
int main(int argc, char *argv[]){
   if (argc<3){
      ...
      printf("Usage: ./sudoku <threadCount> <inputFile>\n");
      exit(0);
   }
   ...
   int **outputGrid = solSudoku(originalGrid);
   ...
   printf("\n\n\n");
   if (checkValid(originalGrid,outputGrid)){
      printf("SUDOKU SOLUTION FOUND\nTIME ELAPSED = %lf\n",(finish-start));
   }
   else{
      printf("NO SOLUTION OF SUDOKU FOUND\nTIME ELAPSED =%lf\n",(finish-start));
   }
}
...
void moveToNextUnsetCell(grid_t grid){
   while(grid->i < SIZE * SIZE && CurrValue(grid)) {
      grid->i++;
   }
}
...
grid_t Alloc_grid(stack_t avail) {
   ...
   if (avail == NULL || Empty_stack(avail)) {
      tmp = (grid_t) malloc(sizeof(gridStruct));
...
stack_t Alloc_stack() {
   ...
   stack_t tmp;
   tmp = (stack_t)malloc(sizeof(stackStruct));
...
int Prepare_allotment_stack(grid_t grid) {
   ...
   do {
      ...
      while(!Empty_stack(allot_stack1)) {
         ...
         curr_grid->i = 0;
         while(curr_grid->i < SIZE * SIZE && CurrValue(curr_grid))
            curr_grid->i++;
         if(curr_grid->i == SIZE * SIZE) {
            // solution has been found
            solFound = 1;
            solMatrix = curr_grid->matrix;
            return 1;
         }
      ...
      j++;
   } while(allot_stack2->list_sz < threadCount);
...
int solveLogical(grid_t grid) {
   ...
   do {
      ...
      // TODO : set solFound
   } while(!solFound && changes);
   return 0;
}


int *solSudoku(int * original_matrix) {
   ...
   // exit(0);
   #pragma omp parallel shared(solFound)
   {
      int tid, i, nthrds;
      int idx, v;
      uint64_t vals;
      ...
      for(i = tid; i < allotment_stack->list_sz && !solFound; i+=nthrds) {
   ...
   if(solFound) {
      for(i = 0; i < SIZE * SIZE; i++) {
         ret_matrix[i/SIZE][i%SIZE] = solMatrix[i];
      }
   }
   else {
